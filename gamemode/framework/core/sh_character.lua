...
ax.character:RegisterVar("skin", {
    ...
    populate = function(this, container, payload)
        ...
        local slider = container:Add("ax.slider")
        slider:Dock(TOP)
        slider:SetTall(ax.util:ScreenScale(14))
        slider:DockMargin(0, 0, ax.ui:Space("md"), ax.ui:Space("lg"))
        slider:SetZPos(this.sortOrder)
        slider:SetMin(0)
        slider:SetMax(math.max(NumModelSkins(payload.model) - 1, 0))
