...
--- Determines the intended realm of a Lua file from its filename prefix. Inspects only the filename component (not the directory path). The check is case-insensitive. Recognised prefixes:
--- - `cl_` → `"client"`
--- - `sv_` → `"server"`
...
---@usage ax.util:DetectFileRealm("sv_database.lua")        -- "server"
---@usage ax.util:DetectFileRealm("sh_config.lua")          -- "shared"
---@usage ax.util:DetectFileRealm("framework/init.lua")     -- "shared"
---@usage ax.util:DetectFileRealm("modules/rq_minstd.lua")  -- "require"
...
--- Includes a Lua file with automatic realm-based routing. Normalises path separators and strips leading slashes, then detects the realm via `DetectFileRealm` if no hint is provided. Routing rules:
...
--- Recursively includes all `.lua` files found under a directory. On the first call (not from recursion), the calling file's directory is detected via `debug.getinfo` and prepended to `directory` to resolve relative paths correctly. Subsequent recursive calls pass `fromLua = true` to skip that detection step. Each `.lua` file is passed to `Include`, which handles realm detection and `AddCSLuaFile`/`include` routing. Subdirectories are processed depth-first. Files and directories listed in `toSkip` (as keys, e.g. `{ ["boot.lua"] = true }`) are silently skipped. The optional `timeFilter` (in seconds) skips files that have not been modified within that time window — useful for selective hot-reload workflows.
---@realm shared
...
--- Reads and parses a JSON file from the `DATA` directory. Reads the file with `file.Read(path, "DATA")`, then deserialises it with `util.JSONToTable` (wrapped in `pcall`). Returns nil without throwing on any failure: missing file, empty file, or malformed JSON. Safe to call speculatively — a missing config file is not an error condition.
---@realm shared
...
--- Serialises a table to JSON and writes it to a file in the `DATA` directory. Creates any missing parent directories automatically before writing. All failure cases (serialisation error, directory creation error, write error) print a warning and return false without throwing. Serialisation uses `util.TableToJSON` wrapped in `pcall`.
---@realm shared
...
--- Builds a `DATA`-relative JSON file path for a given key and scope. `key` is sanitised via `SanitizeKey` to remove filesystem-unsafe characters. Three scope modes are supported via `options.scope`:
...
---@usage ax.util:BuildDataPath("config", { scope = "global" })  -- "global/config.json"
---@usage ax.util:BuildDataPath("zones", { scope = "map" })      -- "parallax/maps/gm_construct/zones.json"
...
--- Creates all directories in a `DATA`-relative path that do not yet exist. Splits `path` into segments by `/` and walks them cumulatively, calling `file.CreateDir` for each segment that is not already a directory. Idempotent — safe to call repeatedly even if the directories already exist. Typically called before writing a file to guarantee its parent directories are present.
---@realm shared
...
--- Loads scripted entities, weapons, tools, and effects from a directory. Scans four subdirectories under `path`: `entities/`, `weapons/`, `tools/`, and `effects/`. For each, both folder-based and single-file layouts are supported:
...
--- Entities and weapons are registered with `scripted_ents.Register` and `weapons.Register` respectively. Tools are injected into `gmod_tool`'s Tool table. Effects are registered client-side only via `effects.Register`. When any tools are loaded, `spawnmenu_reload` is issued on the client to refresh the spawnmenu. The optional `timeFilter` skips entities whose directory or file has not been modified within that many seconds — useful for selective hot-reload passes.
---@realm shared
