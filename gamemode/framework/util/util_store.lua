...
-- Store factory utilities. Provides a canonical pattern for creating configurable stores used by config (server→client broadcast settings) and option (client→server preferences). Stores support registration, defaults, persistence, and optional networking of specific keys.
...
--- Creates and returns a new store instance for managing key/value settings. A store is a self-contained object that handles registration of typed settings, persistence to a JSON file (or `ax.data`), optional networking between server and client, and change callbacks. The framework uses two stores: `ax.config` (server authority, broadcast to clients) and `ax.option` (client authority, synced to server for server-side reads). `spec` fields:
...
function ax.util:CreateStore(spec, oldStore)
    ...
    --- Registers a new key/type/default definition in the store. After registration, the key is available for `Get` and `Set`. The initial value is loaded from persisted data (if available and on the authority side), coerced to the registered type, and stored. If persisted data is absent or fails coercion, the `default` is used. Keys are automatically added to `store.networkedKeys` unless `data.bNoNetworking = true`. `data` fields:
    ...
    --- Retrieves a value from the store with optional fallback. Normal call: `store:Get(key, fallback)` — returns the stored value for `key`, or `store.defaults[key]`, or `fallback` (in that priority order). On the client, config values are served from the local CONFIG_CACHE (populated by server sync) rather than `store.values`. Special call for `option` stores on the server: `store:Get(player, key, fallback)` — reads a per-player option value from SERVER_CACHE (populated when the client sends its preferences). If no cached value exists for that player, the registered default is used.
    ---@realm shared
    ...
    --- Returns a copy of the metadata table for a registered key. Returns a deep copy so callers cannot accidentally mutate the registry. Returns nil when `key` is not a string or is not registered.
    ---@realm shared
    ...
    --- Returns the registered default value for a key. This is the value passed as `default` to `store:Add`, or subsequently updated by `store:SetDefault`. Returns nil when `key` is not a string or is not registered.
    ---@realm shared
    ...
    --- Sets a value in the store, triggering coercion, callbacks, and networking. The full pipeline when setting a value:
    ...
    --- Updates the registered default value for a key without changing the current value. Modifies both the registry entry's `default` field and `store.defaults[key]`. The live value in `store.values` is only changed if it is currently nil — existing values are preserved. Useful for overriding defaults set during initial registration, e.g. when a gamemode wants to change framework defaults. Returns false when the key is not registered.
    ---@realm shared
    ...
    --- Resets a key's value to its registered default. Convenience wrapper around `store:Set(key, default)`. Goes through the full Set pipeline including coercion, callbacks, persistence, and networking — the reset is treated the same as any other value change. Returns false when the key is not registered, or when the current value is already equal to the default (no change).
    ---@realm shared
    ...
    --- Returns an array of all unique category names in the registry. Iterates all registered keys and collects their `data.category` field. Keys without an explicit category are counted under `"general"`. The returned array contains each category string exactly once, in arbitrary order. Useful for building category tabs in a settings UI.
    ---@realm shared
    ...
    --- Returns a deep copy of all registered setting definitions. Each entry in the returned map is a copy of the registry entry (type, default, data) keyed by the setting key. Modifying the returned table does not affect the live registry. Useful for introspecting all available settings, e.g. for building a settings panel or exporting documentation.
    ---@realm shared
    ...
    --- Returns definitions for all keys whose category matches a filter string. Matching is performed by `ax.util:FindString` — case-insensitive substring match. For example, `"gen"` will match `"general"`. Keys with no category fall under `"misc"` for the purposes of this filter. The returned map contains deep copies of the matched registry entries.
    ---@realm shared
    ...
    --- Loads persisted values from disk into the store. Only executes on the authority side (`spec.authority`): server stores load on `SERVER`, client stores load on `CLIENT`. Calling from the non-authority side is a no-op that returns false. Clears the persisted data cache first (`InvalidatePersistedCache`), then loads from the primary path (or `ax.data` if configured). If the primary path is absent, falls back to any `spec.legacyPaths` entries and migrates the data to the primary path on success. Each loaded key is coerced to its registered type; keys that fail coercion are skipped (debug message only). After loading, fires either `hook.Run("OnConfigsLoaded")` or `hook.Run("OnOptionsLoaded")`.
    ---@realm shared
    ...
    --- Persists the current store values to disk. Only executes on the authority side (`spec.authority`). No-op on the non-authority side. Serialises `store.values` and writes it to the primary path via `ax.data:Set` (if configured) or `ax.util:WriteJSON`. On success, updates the persisted cache so subsequent `Add` calls can read back the correct initial value without re-reading the file. A warning is printed (but false is returned silently) if the path is invalid or the write fails. This function is called automatically by `Set` (unless `bNoSave` is true) and on `ShutDown`.
    ---@realm shared
    ...
    --- Synchronises networked key values across the network. Behaviour depends on the store type and current realm:
    ...
    --- Internal: wires up all net channel receivers and lifecycle hooks. Called once during store initialisation. If `ax.net` is not yet available, defers via `timer.Simple(0, ...)` and retries on the next frame. Registers the following depending on store type and realm:
    ...
    --- Also registers a `ShutDown` hook on the authority side to call `Save`. This is an internal method — do not call it directly.
    ---@realm shared
    ...
    --- Invokes the `OnChanged` callback for a key if one is registered. Called by the networking layer after a config value arrives from the server (on the client side). The callback is called via `SafeCall` so errors in user-defined callbacks do not break the networking flow. `oldValue` may be nil when this is the initial sync (no prior value). `regEntry` being nil or having no `OnChanged` function is safe — the call is silently skipped.
    ---@realm shared
