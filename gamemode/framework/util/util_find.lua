...
--- Normalises a value into a lowercase, trimmed string for search comparisons. Converts `value` to a string via `tostring`, trims leading/trailing whitespace, and lowercases the result using `utf8.lower` when available (falling back to `string.lower`). Returns `""` when the result is empty or when `value` is nil. Used as a pre-processing step by `SearchMatches` to ensure consistent case-insensitive matching.
---@realm shared
...
---@usage ax.util:NormalizeSearchString(nil)          -- ""
...
--- Tests whether a search query is contained in any of the provided candidates. Both the query and each candidate are normalised with `NormalizeSearchString` before comparison. An empty query always returns true — this matches the convention of "show everything when the search box is blank". The check is substring-based (not exact match), so `"cit"` will match `"citizen"`.
---@realm shared
---@param query string The search text to look for.
---@param ... any Candidate values to test against (each coerced to string).
---@return boolean # True if the query is empty, or if any candidate contains the query as a substring (case-insensitive).
---@usage ax.util:SearchMatches("pol", "Citizen", "Police Officer") -- true
---@usage ax.util:SearchMatches("", "anything")              -- true
---@usage ax.util:SearchMatches("xyz", "Citizen", "Police")  -- false
...
--- Tests whether `find` appears as a substring inside `str`. By default the search is case-insensitive (both strings are lowercased via `utf8.lower` or `string.lower` before comparison) and pattern characters in `find` are treated as plain text. All optional parameters allow overriding these defaults when needed. Prints an error and returns false when either `str` or `find` is nil.
---@realm shared
---@param str string The string to search in.
---@param find string The substring (or pattern) to search for.
---@param caseSensitive boolean|nil When true, the strings are compared without lowercasing. Default: false (case-insensitive).
---@param startPos number|nil The character position to start searching from. Default: 0 (from the beginning).
---@param usePatterns boolean|nil When true, `find` is treated as a Lua pattern. Default: false (plain-text search via `string.find` plain flag).
---@return boolean # True if `find` appears in `str`, false otherwise.
---@usage ax.util:FindString("Hello World", "world")         -- true
---@usage ax.util:FindString("Hello World", "World", true)   -- true (exact case)
---@usage ax.util:FindString("Hello World", "World", false)  -- true (case-insensitive)
---@usage ax.util:FindString("Hello World", "xyz")           -- false
...
---@usage ax.util:FindText("hello\nworld", "world")         -- true
---@usage ax.util:FindText("hello world", "xyz")            -- false
...
--- Finds a connected player by a variety of identifier types. Lookup is attempted in this priority order:
...
--- 4. If `identifier` is a table (array), each element is tried recursively and the first successful match is returned.
--- Returns `NULL` (not nil) on no match — callers should test with `IsValid`.
---@realm shared
---@param identifier number|string|Player|table An entity index, SteamID, SteamID64, name substring, a Player entity, or a table of any of these.
---@return Player # The matched player entity, or `NULL` (not nil) if not found.
---@usage ax.util:FindPlayer("76561198000000000")
---@usage ax.util:FindPlayer("John")  -- partial name match
---@usage ax.util:FindPlayer(1)       -- entity index
...
--- Finds a character by numeric ID or by name (partial, case-insensitive). Searches in two passes:
...
---@realm shared
---@param identifier number|string A numeric character ID or a name string (full or partial).
---@return table|nil # The matched character object, or nil when nothing matched.
---@usage ax.util:FindCharacter(42)          -- by ID
---@usage ax.util:FindCharacter("John")      -- partial name match
---@usage ax.util:FindCharacter("john doe")  -- case-insensitive
