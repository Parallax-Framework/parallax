...
--- Converts between a JSON string and a Lua table safely. Two modes depending on the inputs:
...
--- Calls a function safely, capturing any Lua error without throwing. NOTE: This definition is overwritten later in this file by a second, simpler SafeCall. See the authoritative definition below for the live behaviour. This version prints the error via `PrintError` and unpacks all return values; the later version returns only the first return value.
---@realm shared
...
--- Converts a human-readable name into a lowercase, underscore-separated ID. Applies three transformations in order:
...
---@usage ax.util:NameToUniqueID("MyModule")    -- "my_module"
---@usage ax.util:NameToUniqueID("Faction 01")  -- "faction_" (digits stripped)
...
--- Converts an underscore-separated unique ID to PascalCase. Each underscore followed by a lowercase letter triggers capitalisation of that letter and removal of the underscore. The very first character is also capitalised. Any character that is not alphanumeric is then stripped from the final result. Returns an empty string when `id` is not a string.
---@realm shared
...
---@usage ax.util:UniqueIDToCamel("item_type_id")  -- "ItemTypeId"
...
--- Converts an underscore-separated unique ID back into a human-readable name. Applies three transformations in order:
...
---@usage ax.util:UniqueIDToName("player_health_max")  -- "Player Health Max"
...
--- Converts a key/value map into a sorted array of entry objects. Each entry in the returned array is a table `{ key = k, value = v }`. The default sort order is case-insensitive lexicographic by key; when two keys are equal after lowercasing, the original (case-sensitive) key string is used as a tiebreaker so the order is stable and predictable. Provide a custom `comparator(a, b)` (where `a` and `b` are entry tables) to override the sort. Returns an empty table if `entries` is not a table.
---@realm shared
...
--- Returns a cached `IMaterial`, creating it on first use. Materials are cached by a compound key of `path` and `parameters` so that the same path with different parameter strings produces distinct cache entries. `parameters` defaults to `""` — callers that omit it get a different cache slot from those that pass an empty string explicitly only if they use a different string value. Prints an error and returns false when `path` is nil/false.
---@realm shared
...
if ( CLIENT ) then
    --- Draws a blurred, optionally rounded rectangle on the screen. Thin wrapper around `ax.render.Draw` using `ax.render.BLUR` mode. The blur is rendered over whatever is already on screen at that region, making it useful for frosted-glass UI elements. Alpha on `color` controls the overall opacity of the blur overlay.
    ---@realm client
    ...
    --- Resolves a short gradient direction name to a GMod vgui material path. Accepted short names and their aliases (case-insensitive):
    ...
    ---@usage ax.util:GetGradientPath("vgui/my_gradient")  -- "vgui/my_gradient"
    ...
    --- Returns a cached gradient `IMaterial` by direction name or path. Resolves `name` to a path via `GetGradientPath`, then fetches or creates the material via `GetMaterial`. Subsequent calls with the same name are served from the material cache at no extra cost.
    ---@realm client
    ...
    --- Draws a gradient material tinted with a color on the screen. Resolves the gradient via `GetGradient(name)`, then draws it using `ax.render.DrawMaterial` with optional corner rounding and tint color. When `color` is omitted, an opaque white tint is used (no tinting).
    ---@realm client
    ---@param r number Corner roundness in pixels (0 for a sharp rectangle).
...
--- Clamps a number to a range, then rounds it to a given precision. Clamping is applied first, so the rounded result is always within `[min, max]`. When `decimals` is 0 or omitted, `math.Round` is used (nearest integer). For positive `decimals`, the number is scaled by `10^decimals`, rounded, then scaled back, preserving that many decimal places. `min` and `max` are both optional; pass nil to skip either bound. Non-numeric input for `n` is treated as 0.
---@realm shared
...
---@usage ax.util:ClampRound(150, 0, 100)        -- 100
---@usage ax.util:ClampRound(4.6)                -- 5
...
--- Scales a 0–1 progress fraction by a smoothness percentage. This is a low-level helper used by `ApproachNumber`, `ApproachVector`, and `ApproachAngle`. `smooth` is expressed as a percentage (0–100): a value of 100 leaves the fraction unchanged; lower values compress it toward 0, making transitions feel slower or more eased. The result is always clamped to [0, 1] regardless of input. `linear` is accepted for signature compatibility but does not currently alter the calculation — the distinction between linear and non-linear movement is handled at the `ApproachNumber` level.
---@realm shared
...
--- Interpolates a number from `start` to `finish` using a progress fraction. The fraction is first passed through `ApproachFraction` using `opts.smooth` (default 100, i.e. no compression). The transition mode then determines how the adjusted fraction maps to the final value:
...
--- Interpolates a Vector component-wise from `startVec` to `finishVec`. Delegates to `ApproachNumber` independently for X, Y, and Z and assembles the results into a new `Vector`. All transition modes and smoothness options supported by `ApproachNumber` work here too. Useful for smoothly animating world positions or velocities over time in a `Think` hook.
---@realm shared
---@param fraction number Progress fraction in [0, 1].
...
--- Interpolates an Angle component-wise from `startAng` to `finishAng`. Delegates to `ApproachNumber` independently for pitch (p), yaw (y), and roll (r) and assembles the results into a new `Angle`. Note that raw lerping of angles can wrap unexpectedly near 180°/−180° boundaries; for large yaw sweeps, normalise the angles before calling this function.
---@realm shared
---@param fraction number Progress fraction in [0, 1].
...
--- Returns true if `client` is a player entity. Checks the Lua type string via `type()` rather than `IsValid()`, so it is safe to call on any value including nil, non-entity tables, or invalid entities (which would make `IsValid()` error). Use this instead of `IsValid(client) and client:IsPlayer()` when you don't yet know whether the value is even an entity.
---@realm shared
...
--- Returns the world position and angle of an entity's head. Checks for the `"eyes"` attachment first; if found and valid, its position and angle are returned. Falls back to the `ValveBiped.Bip01_Head1` bone when the attachment is absent or returns no data. Returns nil for both values when neither source is available (e.g. a prop with no skeleton). A debug axis overlay is drawn at the resolved position for 0.1 seconds — this is intentional for development use.
---@realm shared
...
--- Sanitises an arbitrary string so it is safe to use in file names and paths. Any character that is not alphanumeric (`A–Z`, `a–z`, `0–9`), a hyphen (`-`), underscore (`_`), or period (`.`) is replaced with an underscore. The value is coerced to string via `tostring` before processing. Returns `""` when `key` is nil or false.
---@realm shared
...
---@usage ax.util:SanitizeKey("config/sub dir/file")  -- "config_sub_dir_file"
---@usage ax.util:SanitizeKey("my.setting_key-v2")    -- "my.setting_key-v2"
...
--- Returns the active gamemode's folder name, used as a data namespace. Calls `engine.ActiveGamemode()` to read the folder name of the gamemode currently running (e.g. `"parallax"` or `"parallax-militaryrp"`). Falls back to the literal string `"parallax"` if the engine API is unavailable. This value is used by functions like `BuildDataPath` to scope persisted data files per-gamemode so derived gamemodes don't collide with each other.
---@realm shared
...
--- Returns the server's network address, IP, and port as separate values. Calls `game.GetIPAddress()` first. On the server side, falls back to the `sv_ip` convar if the primary call returns an empty string. Parses the resulting `"ip:port"` string into its components; the port defaults to 0 when not present in the string. Returns three nils when no address can be determined, and logs a debug message.
---@realm shared
...
--- Returns the surface data table for a named physics surface material. Looks up the internal surface index with `util.GetSurfaceIndex`, then retrieves the data table via `util.GetSurfaceData`. Returns nil when the name is not registered (index 0) or when `surfaceName` is nil/false. Surface names are defined in `scripts/surfaceproperties.txt` and its includes (e.g. `"concrete"`, `"metal"`, `"wood"`).
---@realm shared
...
--- Returns the surface data table for the surface hit by a trace result. Reads `tr.SurfaceProps` (the internal physics surface index stored in every trace result) and forwards it to `util.GetSurfaceData`. Returns nil when the trace missed (`tr.Hit` is false), when `tr` is nil, or when `SurfaceProps` is 0 (no valid surface). Pair with `util.TraceLine` or `util.TraceEntity` to get the trace result.
---@realm shared
...
if ( CLIENT ) then
    --- Scales a pixel value by the player's UI scale preference. Reads the `interface.scale` option (default 1.0) and multiplies `value` by it. Use this for any hardcoded pixel dimension that should respect the user's interface scale setting (panel sizes, font sizes, offsets, etc.).
    ---@realm client
    ...
    --- Applies GMod's `ScreenScale` then multiplies by the player's UI scale. `ScreenScale` converts a value designed for a 640-wide reference display to the current screen width. This function additionally applies the `interface.scale` option on top, so sizes remain consistent across both different resolutions and different user scale preferences. Prefer this over raw `ScreenScale` for all UI element sizing.
    ---@realm client
    ...
    --- Applies GMod's `ScreenScaleH` then multiplies by the player's UI scale. The height-based equivalent of `ScreenScale` — scales a reference value relative to screen height (480 px reference) rather than width. Multiply by the `interface.scale` option afterward for consistent vertical sizing.
    ---@realm client
...
--- Pads a number with leading zeroes to a minimum digit count. Uses `string.format` with `%0Xd` under the hood. The result is always a string. If the number already has more digits than `digits`, it is returned without truncation — `digits` is a minimum, not a fixed width.
---@realm shared
...
---@usage ax.util:PadNumber(1234, 3)  -- "1234" (already wider, not truncated)
---@usage ax.util:PadNumber(0, 2)     -- "00"
function ax.util:PadNumber(num, digits)
    return string.format("%0" .. tostring(digits) .. "d", num)
end

--- Returns true when `target` falls within `client`'s crosshair cone. Uses the dot product of `client`'s aim vector and the normalised direction to `target`'s eye position (or world centre for non-player entities). The dot product is compared against `range` as a threshold: a value of `1.0` means the target must be directly on-axis; `0.9` (default) allows roughly ±26°; `0.0` accepts any direction in front of the player. A lower `range` value produces a wider acceptance cone. Returns nil (falsy) when either `client` or `target` is not a valid player.
---@realm shared
...
--- Asserts a condition, printing an error message when it is false. Unlike Lua's built-in `assert`, this does NOT throw — execution continues after the call. The error is printed via `PrintError` (which uses `ErrorNoHaltWithStack`, so a stack trace is included in the output). When `condition` is falsy, returns `false` followed by any extra arguments. When `condition` is truthy, returns it unchanged followed by extra arguments. Use this for precondition checks where you want to log the failure clearly but still allow the caller to handle it gracefully.
---@realm shared
...
--- Returns the player whose death ragdoll is the given entity. Iterates all connected players and checks whether any has a `ragdoll.index` relay value matching the entity's index. This relay is set by the framework when a player's death ragdoll is created. Returns nil when the entity is not a `prop_ragdoll`, is invalid, or no player has that ragdoll index.
---@realm shared
...
--- Asserts a condition, printing a debug message when it is false. Behaves identically to `Assert` except the failure message is routed through `PrintDebug` instead of `PrintError`. This means the output is only visible when the `developer` convar is ≥ 1 AND `ax_debug_realm` is set to match the current realm — making it suitable for internal consistency checks that should be silent in production but informative during development.
---@realm shared
...
if ( CLIENT ) then
    --- Draws a smooth arc or full circle outline using polygons. Generates vertices along the arc from `startAngle` to `endAngle` (both in degrees) and draws them as a polygon using `surface.DrawPoly`. The arc is not filled — it is an open polyline. For a filled circle, use `DrawSlice` instead. `segments` controls smoothness; 64 is a good default for most sizes. Call `surface.SetDrawColor` before this function to set the line colour.
    ---@realm client
    ...
    --- Draws a filled pie slice, useful for circular progress indicators. Constructs a polygon with the centre point as vertex 0 and arc vertices fanning out from `startAngle` to `endAngle`. An internal –90° offset is applied so that 0° corresponds to the top of the circle (12-o'clock position), making it intuitive for progress bars that start at the top. Uses a fixed 64-segment resolution. The slice is filled solid with `color`.
    ---@realm client
