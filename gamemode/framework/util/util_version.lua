...
--- Returns the Parallax framework version string (e.g. `"0.3.42"`). Performs a two-tier lookup:
...
--- Returns the short Git commit hash for this Parallax build. Performs the same two-tier lookup as `GetVersion`: live `ax.version` data first, then `ReadVersionFile()` as a fallback. Falls back to an empty string `""` when unavailable, so this function always returns a string. The hash is useful for identifying the exact source revision when reporting bugs or comparing server builds.
---@realm shared
