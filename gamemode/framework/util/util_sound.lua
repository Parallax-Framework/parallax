...
--- Registers a named sound script and precaches it for use in gameplay. Wraps `sound.Add` with validated inputs and sane defaults. After registration the sound is immediately precached via `util.PrecacheSound`, so callers do not need to call that separately. Sound scripts registered here can be emitted with `Entity:EmitSound(name)`. Defaults when parameters are omitted:
...
--- `path` may be a string for a single file, or a table of file paths for random variation (GMod picks one at random per emission). `pitch` may be a fixed number (e.g. 100) or a `{ min, max }` table for random pitch per emission (e.g. `{ 95, 105 }`). Returns false with a printed error when `name` or `path` is invalid.
---@realm shared
