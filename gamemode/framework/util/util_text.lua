...
--- Truncates text to a maximum character count, appending `"..."` when cut. If the text is already within `maxLength` characters it is returned unchanged. When truncation is needed, the text is cut to `maxLength - 3` characters and `"..."` is appended, so the returned string is always at most `maxLength` characters long in total. Returns `""` and prints an error when either argument is invalid.
---@realm shared
...
---@usage ax.util:CapText("Hi", 10)            -- "Hi" (unchanged)
...
--- Truncates text at a word boundary to avoid splitting words mid-character. Splits the text by spaces and accumulates words until the next word would exceed `maxLength`. The result always has `"..."` appended, even when the boundary falls exactly at `maxLength`. As a result the returned string may be shorter than `maxLength` if the last fitting word ends well before it. Returns `""` and prints an error when either argument is invalid.
---@realm shared
---@param text string The text to truncate.
...
---@usage ax.util:CapTextWord("hi", 50)                   -- "hi" (unchanged)
...
if ( CLIENT ) then
    ...
    --- Returns the rendered pixel width of a string in a given font. Calls `surface.SetFont(font)` as a side effect, changing the active surface font state. Use `GetTextSize` if you need both dimensions at once.
    ---@realm client
    ...
    --- Returns the line height of a font in pixels. Measures the height of the capital letter `"W"` as a representative character that occupies the full ascender height of the font. The result is the number of vertical pixels a single line of text in this font occupies. Calls `surface.SetFont(font)` as a side effect.
    ---@realm client
    ...
    --- Returns the rendered pixel dimensions of a string in a given font. Convenience wrapper that calls `surface.SetFont(font)` then returns both values from `surface.GetTextSize(text)` in one call. Use this when you need both width and height to avoid two separate font sets.
    ---@realm client
    ---@param font string The font name to measure with.
    ---@param text string The string to measure.
