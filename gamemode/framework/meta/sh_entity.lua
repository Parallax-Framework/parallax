...
--- Returns true if the entity's model is a chair from the Vehicles list. The check is performed against a pre-built lookup table (`MODEL_CHAIRS`) populated at file load time from `list.Get("Vehicles")`, filtered to the `"Chairs"` category. Comparison is case-insensitive.
---@realm shared
...
--- Returns true if the entity's model class contains the substring "female". The model class is retrieved via `ax.animations:GetModelClass(model)`. If the model class is not a valid non-empty string, the function returns false. Otherwise, it performs a case-insensitive substring search for "female" within the model class name.
---@realm shared
...
--- Enforces a named rate limit on this entity. Stores timestamps in `entity.axRateLimits[name]`. On each call:
...
--- When `delay` is 0 or omitted, no timestamp is recorded and the call always returns true (one-shot check with no cooldown). The rate limit state persists on the entity table across calls. Prints an error and returns false when `name` is invalid.
---@realm shared
...
--- Clears a named rate limit so the next call to `RateLimit` passes immediately. Removes the stored expiry timestamp for `name` from `entity.axRateLimits`. Safe to call even if the limit is not set or has already expired. Returns true on success, false on invalid input.
---@realm shared
...
--- Returns whether the entity is in a locked state. Reads the internal engine variable that tracks lock state: `m_bLocked` for doors, vehicles and other lockable props.
---@realm shared
