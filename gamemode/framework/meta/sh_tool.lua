...
-- Replicated from the Sandbox tool base (`stool.lua`). Initialises all standard
-- fields with their defaults: `Mode`, `SWEP`, `Owner`, `ClientConVar`,
-- `ServerConVar`, `Objects`, `Stage`, `Message`, `LastMessage`, and `AllowedCVar`.
...
-- On the client, iterates `self.ClientConVar` and calls `CreateClientConVar` for each
-- entry prefixed with `"<mode>_"`. On the server, creates the `"toolmode_allow_<mode>"`
-- convar (with `FCVAR_NOTIFY`) which controls whether this tool mode is permitted on
-- the server. This is called automatically during tool registration.
-- @realm shared
...
-- Reads the convar named `"<mode>_<property>"` via `GetConVarString`. Useful for
-- reading server-authoritative settings from within shared or client code.
-- @realm shared
...
-- Iterates `self.ClientConVar` and builds a new table keyed by `"<mode>_<k>"` with
-- the corresponding default values. Useful for syncing all tool convars at once.
-- @realm shared
...
-- Reads the convar named `"<mode>_<property>"` from the owning player via
-- `Player:GetInfo`. This reflects the client's locally set value.
-- @realm shared
-- @param property string The convar suffix (the part after `"<mode>_"`).
...
-- Reads the convar named `"<mode>_<property>"` from the owning player via
-- `Player:GetInfoNum`. Falls back to `default` (converted to number, or 0) when
-- the convar is unset or non-numeric.
-- @realm shared
-- @param property string The convar suffix (the part after `"<mode>_"`).
...
-- On the server, reads the `"toolmode_allow_<mode>"` convar created by
-- `CreateConVars`; returns its boolean value.
-- @realm shared
...
-- Empty by default; override this in a tool definition to run setup logic when the
-- tool object is created. Equivalent to the `Init` stub in the Sandbox tool base.
-- @realm shared
function TOOL:Init()
end

--- Returns the mode string identifying this tool.
-- The mode string is the tool's registered name (e.g. `"axis"`, `"weld"`). It is used
-- as the prefix for all convar names associated with this tool.
-- @realm shared
...
-- The SWEP is the `gmod_tool` weapon entity in the player's hand. Access the owning
-- player via `GetOwner()` rather than reading `SWEP.Owner` directly.
-- @realm shared
...
-- Reads `SWEP.Owner` first, falling back to `self.Owner` when the SWEP reference is
-- unavailable. This is the player who has the `gmod_tool` weapon equipped.
-- @realm shared
...
-- Reads `SWEP.Weapon` first, falling back to `self.Weapon`. Equivalent to the
-- `gmod_tool` weapon entity rather than the owning player.
-- @realm shared
...
-- Returns false by default (no action taken). Override in a tool definition to
-- implement the primary tool interaction (e.g. placing, welding, constraining).
-- @realm shared
...
-- Returns false by default (no action taken). Override in a tool definition to
-- implement the secondary tool interaction.
-- @realm shared
-- @return boolean True if the click was handled, false otherwise.
...
-- Clears all stored objects via `ClearObjects`. Override to add additional reset
-- behaviour, but ensure `ClearObjects` is still called to maintain consistent state.
-- @realm shared
...
-- Releases any ghost entity via `ReleaseGhostEntity`. Override to show a preview
-- ghost or run setup logic when the player switches to this tool.
-- @realm shared
...
-- Releases any ghost entity via `ReleaseGhostEntity`. Override to clean up any
-- visual state or timers created during `Deploy`.
-- @realm shared
...
-- Releases any ghost entity by default. Override to implement per-frame ghost
-- preview updates; call `ReleaseGhostEntity` only when no ghost should be shown.
-- @realm shared
function TOOL:Think()
    self:ReleaseGhostEntity()
end
...
-- Iterates `self.Objects` and calls `ClearObjects` if any entry's `Ent` field is
-- neither a valid entity nor the world. This prevents operations on stale entity
-- references (e.g. when a prop is deleted mid-interaction).
