...
function ax.util:CreateStore(spec)
    ...
    function store:Add(key, typeId, default, data)
        if not isstring(key) or not typeId or default == nil then
    ...
    function store:Get(...)
        ...
        if spec.name == "ax.option" and SERVER and IsValid(key) and key:IsPlayer() then
            ...
            if not ax.util:IsValidPlayer(client) then
                ax.util:PrintDebug(spec.name, "Get: Invalid player provided")
                return fallback
            end

            if not SERVER_CACHE[client] or SERVER_CACHE[client][actualKey] == nil then
        ...
        if not isstring(key) then return default end
    ...
    function store:Set(key, value, bNoSave)
        ...
        if regEntry.typeId == ax.type.array and isfunction(regEntry.data.populate) then
            local ok, choices = ax.util.safe_call(regEntry.data.populate)
            if ok and istable(choices) and not choices[coerced] then
        ...
        if isfunction(regEntry.data.OnChanged) then
            ax.util.safe_call(regEntry.data.OnChanged, oldValue, coerced, key)
        end
    ...
    function store:SetDefault(key, value)
        ...
        local regEntry = store.registry[key]
        if not regEntry then return false end
    ...
    function store:GetAllByCategories(bRemoveHidden)
        ...
        for key, entry in pairs(store.registry) do
            ...
            if bRemoveHidden and isfunction(data.hidden) then
                local ok, isHidden = ax.util.safe_call(data.hidden)
                if ok and isHidden then continue end
            end
    ...
    function store:Load()
        if not ((spec.authority == "server" and SERVER) or (spec.authority == "client" and CLIENT)) then
            return false
        end
    ...
    function store:Save()
        ...
        local data = {}
        for key, value in pairs(store.values) do data[key] = value end
    ...
    function store:Sync(target)
        if spec.name == "ax.config" and SERVER then
            ...
            if not istable(recipients) then recipients = {recipients} end
            local networked = {}
            for key in pairs(store.networkedKeys) do networked[key] = store.values[key] end
            if table.Count(networked) > 0 then
                net.Start(spec.net.init)
                net.WriteTable(networked)
                for _, client in ipairs(recipients) do if ax.util:IsValidPlayer(client) then net.Send(client) end end
    ...
    function store:_setupNetworking()
        if spec.name == "ax.config" then
            if SERVER then
                ...
                hook.Add("PlayerInitialSpawn", "ax.config.Init", function(client)
                    timer.Simple(1, function() if ax.util:IsValidPlayer(client) then store:Sync(client) end end)
                end)
            elseif CLIENT then
                net.Receive(spec.net.init, function()
                    ...
                    for key, value in pairs(data) do
                        ...
                        local regEntry = store.registry[key]
                        if regEntry and isfunction(regEntry.data.OnChanged) then ax.util.safe_call(regEntry.data.OnChanged, nil, value, key) end
                    end
                ...
                net.Receive(spec.net.set, function()
                    ...
                    CONFIG_CACHE[key] = value
                    local regEntry = store.registry[key]
                    if regEntry and isfunction(regEntry.data.OnChanged) then ax.util.safe_call(regEntry.data.OnChanged, oldValue, value, key) end
        ...
        elseif spec.name == "ax.option" then
            if SERVER then
                ...
                net.Receive(spec.net.sync, function(len, client)
                    if not ax.util:IsValidPlayer(client) then return end
                    local data = net.ReadTable()
                    SERVER_CACHE[client] = SERVER_CACHE[client] or {}
                    for key, value in pairs(data) do SERVER_CACHE[client][key] = value end
                ...
                hook.Add("PlayerDisconnected", "ax.option.Cleanup", function(client) SERVER_CACHE[client] = nil end)

                local function requestSync(client)
                    if ax.util:IsValidPlayer(client) then net.Start(spec.net.request); net.Send(client) end
                end
                store.RequestPlayerSync = requestSync

            elseif CLIENT then
                net.Receive(spec.net.request, function() store:Sync() end)
                hook.Add("InitPostEntity", "ax.option.AutoSync", function() timer.Simple(2, function() store:Sync() end) end)
            end
        end
    end
