...
local function drawDeveloperInfo()
    ...
    if mx and my then
        developerText(
            ...
            0, 20, devColors["mouseHovered"]
        )
    else
        developerText(
            string.format("mouse: %s", gState._devInputBlocker or ""),
            0, 20, devColors["mouseUnhovered"]
        )
    end
    ...
    developerText(
        ...
        0, 53, devColors["distance"]
    )

    local ang = gState.angles
    developerText(string.format("ang: %.2f %.2f %.2f", ang.p, ang.y, ang.r), 0, 75, devColors["ang"])
    developerText(string.format("dot %d", gState._devDot or 0), 0, 88, devColors["dot"])
    ...
    if IsValid(ent) and ent._imguiRBExpansion then
        ...
        render.DrawWireframeBox(vector_origin, angle_zero, minrb, maxrb, devColors["renderBounds"])
    end
end
...
local hoverStored = {}
local unHoverStored = {}
function imgui.Button(x, y, w, h, paint, callbackPress, callbackHover, callbackUnHover)
    local uniqueID = string.format("imgui_button_%d_%d_%d_%d", x, y, w, h)
    if not pressStored[uniqueID] then pressStored[uniqueID] = false end
    if not hoverStored[uniqueID] then hoverStored[uniqueID] = false end
    if not unHoverStored[uniqueID] then unHoverStored[uniqueID] = false end

    if paint then
        paint(w, h, pressStored[uniqueID], hoverStored[uniqueID], unHoverStored[uniqueID])
    end

    if not imgui.IsHovering(x, y, w, h) then
        if hoverStored[uniqueID] then
            hoverStored[uniqueID] = false
            if callbackUnHover then
                callbackUnHover()
            end
        end
        return false
    ...
    if not hoverStored[uniqueID] then
        hoverStored[uniqueID] = true
        if callbackHover then
            callbackHover()
        end
    end

    if not imgui.IsPressing() then
        if pressStored[uniqueID] then
            pressStored[uniqueID] = false
            return false
        end
        return true
    end

    if not pressStored[uniqueID] then
        pressStored[uniqueID] = true
