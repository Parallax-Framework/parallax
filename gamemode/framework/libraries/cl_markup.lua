...
local function ExtractParams(p1,p2,p3)
	...
	if (utf8.sub(p1, 1, 1) == "/") then

		local tag = utf8.sub(p1, 2)
...
function ax.markup.Parse(ml, maxwidth)
	...
	for i = 1, #blocks do
		...
		if (block.text) then
			...
			for j=1,utf8.len(block.text) do
				local ch = utf8.sub(block.text,j,j)
				...
				elseif (ch == "\t") then
					...
					if (maxwidth and maxwidth > x) then
						if (xOffset + xSize + x >= maxwidth) then
							...
							local lastSpacePos = utf8.len(curString)
							for k=1,utf8.len(curString) do
								local chspace = utf8.sub(curString,k,k)
								if (chspace == " ") then
									lastSpacePos = k
								end
							end

							if (lastSpacePos == utf8.len(curString)) then
								ch = utf8.sub(curString,lastSpacePos,lastSpacePos) .. ch
								j = lastSpacePos
								curString = utf8.sub(curString, 1, lastSpacePos-1)
							else
								ch = string.utf8.sub(curString,lastSpacePos+1) .. ch
								j = lastSpacePos+1
								curString = string.utf8.sub(curString, 1, lastSpacePos)
							end

							local m = 1
							while utf8.sub(ch, m, m) == " " and m <= utf8.len(ch) do
								m = m + 1
							end
							ch = utf8.sub(ch, m)
