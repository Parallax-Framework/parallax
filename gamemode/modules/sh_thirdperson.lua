...
function MODULE:ShouldUseThirdPerson(client)
    if ( !client:Alive() or client:InVehicle() or client:GetObserverMode() != OBS_MODE_NONE or !client:Alive() or client:InNoclip() ) then return false end
...
ax.viewstack:RegisterModifier("thirdperson", function(client, patch)
    ...
    if ( ax.option:Get("thirdperson.follow.head") ) then
        ...
        if ( bHasRagdoll ) then
            startPos = ax.util:GetHeadTransform(ragdoll) or startPos
        end
    end
    ...
    local traceCamera = util.TraceHull({
        ...
        filter = function(ent)
            ...
            if ( bHasRagdoll and ent == ragdoll ) then return false end
            return true
        end,
        mask = MASK_SOLID
    })
